...
interface AuthModalProps {
  ...
  onAuthenticate: (code: string) => Promise<{ success: boolean; error?: string }>
}
 ...
 */
function formatRecoveryCodeInput(value: string): { formatted: string; cursorOffset: number } {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    requestAnimationFrame(() => {
      if (inputRef.current) {
        ...
        if (formatted.length > input.length && (cursorPosition === 5 || cursorPosition === 10)) {
